...
export default function SidebarLink({ item }) {
  ...
  return (
    <li>
      <Link
        to={item.url}
        className={`sidebar-link ${sidebarState} flex text-lg items-center justify-between hover:bg-blue-600 hover:text-white duration-200 rounded-md md:flex-row-reverse lg:flex-row dark:hover:bg-blue-600 dark:hover:text-white
        ${
      ...
      >
        ...
        >
          ...
          {item.count && item.count > 0 ? (
            <div className="absolute inline-flex items-center p-[10px] justify-center w-5 h-5 text-xs font-semibold text-white bg-blue-600 border-2 border-white rounded-full  end-1 dark:border-gray-900  dark:bg-blue-600 counter">
              {item.count}
            </div>
          ) : null}
        </div>
